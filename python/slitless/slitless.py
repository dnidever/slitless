...
def slitless 
    ...
    str.x = int(np.round(str.x) 
    str.y = randomu(seed,nstars)*(ny-200)+100 
    str.y = int(np.round(str.y) 
    str.flux = randomu(0,nstars)*1e4+200 
    teff = [3500,3750,4000,4500] 
    logg = [2.5,3.0,2.5,3.0] 
    feh = [-1.0,-0.5,-0.5,0.0] 
    str.teff = teff[int(np.round(randomu(0,nstars)*3)] 
    str.logg = logg[int(np.round(randomu(0,nstars)*3)] 
    str.feh = feh[int(np.round(randomu(0,nstars)*3)] 
    ...
    for i in range(nstars): 
        ...
        If dispersion == 0.0: 
            ...
            if str1.feh >= 0.0: 
                ...
                msign='-' 
            synfile = specdir+'spec.t'+string(str1.teff,format='(I4)')+'g'+string(str1.logg,format='(F4.2)')+                         'm'+msign+string(abs(str1.feh),format='(F4.2)')+'a+0.00.fits' 
            if os.path.exists(synfile) == 0 : 
                import pdb; pdb.set_trace(),synfile+' NOT FOUND' 
